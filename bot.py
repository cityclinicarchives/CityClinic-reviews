...
def positive_review_keyboard(review_text: str) -> InlineKeyboardMarkup:
    """
    Кнопка «Скопировать отзыв» использует Telegram copy_text.
    Если в конкретном клиенте Telegram копирование не поддерживается,
    пациент все равно видит текст отзыва в сообщении и может скопировать его вручную.
    """
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
...
async def get_chat_id(message: types.Message) -> None:
    """Показывает ID текущего чата. Нужно для ADMIN_GROUP_ID."""
    await message.answer(f"Chat ID: <code>{message.chat.id}</code>")


@router.message(Command("reviewed"))
async def show_reviewed_patients(message: types.Message) -> None:
    """Показывает телефоны пациентов, уже оставивших отзыв."""
    ...
    lines = []
    for item in rows:
        name = item.get("patient_name") or "без имени"
        phone = item.get("original_phone") or item.get("phone_normalized")
        rating = item.get("rating") or "-"
        created_at = (item.get("created_at") or "")[:19].replace("T", " ")
        lines.append(
            f"• <code>{html.escape(phone)}</code> — {html.escape(name)}, оценка: {rating}, дата: {html.escape(created_at)}"
        )

    header = "Номера пациентов, уже оставивших отзыв:\n\n"
    text = header + "\n".join(lines)
...
async def delete_reviewed(message: types.Message) -> None:
    """Удаляет телефон из базы уже оставивших отзывов. Формат: /delete_reviewed +79991234567"""
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) < 2 or not parts[1].strip():
        await message.answer(
            "Укажите номер телефона после команды. Пример:\n"
    ...
    if deleted:
        ...
        await message.answer(f"Такой номер не найден в базе: <code>{html.escape(phone)}</code>")
...
async def finish_positive_review(message: types.Message, state: FSMContext, review_text: str) -> None:
    ...
    await message.answer(instruction, reply_markup=positive_review_keyboard(review_text))
