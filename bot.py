...
def yandex_review_keyboard(review_text: str) -> InlineKeyboardMarkup:
    """
...
async def finish_positive_review(message: types.Message, state: FSMContext, review_text: str) -> None:
    ...
    if len(review_text) <= 256:
        copy_note = "Нажмите кнопку «Скопировать отзыв» ниже. После этого текст отзыва будет скопирован."
    else:
        copy_note = (
            "Текст отзыва длиннее 256 символов, поэтому автоматическая кнопка копирования "
            "недоступна. Выделите и скопируйте текст отзыва вручную."
        )
