...
def send_cashback_email_sync(patient_name: str, to_email: str) -> None:
    if not settings.email_smtp_user or not settings.email_smtp_password:
        raise RuntimeError("Не заданы EMAIL_SMTP_USER / EMAIL_SMTP_PASSWORD в Railway Variables")

    msg = EmailMessage()
    msg["Subject"] = cashback_email_subject()
    msg["From"] = formataddr(("СитиКлиник Коньково", settings.email_from))
    msg["To"] = to_email
    msg.set_content(
    ...
    msg.add_alternative(cashback_email_html(patient_name), subtype="html")
...
async def send_cashback_email(patient_name: str, to_email: str) -> None:
    await asyncio.to_thread(send_cashback_email_sync, patient_name, to_email)
