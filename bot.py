...
async def finish_positive_review(session: dict, review_text: str) -> None:
    ...
    await wazzup_send_to_patient(session, instruction)
...
async def wazzup_webhook(request: Request) -> Response:
    """
    Webhook для входящих сообщений Wazzup.

    Важно: при подключении webhook Wazzup отправляет тестовый POST.
    На любой тестовый или некорректный payload отвечаем пустым 200 OK,
    иначе Wazzup не принимает URL webhook.
    """
    try:
        payload = await request.json()
    except Exception as exc:
        print(f"Wazzup webhook ignored: invalid json: {exc}")
        return Response(status_code=200)

    print(f"Wazzup webhook payload: {payload}")

    # Тестовый запрос Wazzup при подключении webhook.
    if payload.get("test") is True:
        return Response(status_code=200)

    messages = payload.get("messages") or []
    if not isinstance(messages, list):
        print(f"Wazzup webhook ignored: messages is not list: {payload}")
        return Response(status_code=200)
    ...
    for message in messages:
        ...
        session = await get_active_session_by_phone(
            phone,
        ...
        if not text:
            ...
            if audio_url:
                try:
                    ...
                    await wazzup_send_to_patient(
                        session,
                        "Не получилось распознать голосовое сообщение. Пожалуйста, отправьте текстом.",
                    )
                    processed += 1
                    continue
    ...
    print(f"Wazzup webhook processed messages: {processed}")
    return Response(status_code=200)
