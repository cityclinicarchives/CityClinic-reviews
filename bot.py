import asyncio
import html
import os
from pathlib import Path
from typing import Any
from uuid import uuid4

import httpx
import uvicorn
from aiogram import Bot, Dispatcher, F, Router, types
from aiogram.client.default import DefaultBotProperties
from aiogram.enums import ParseMode
from aiogram.filters import Command, CommandStart
from aiogram.fsm.context import FSMContext
from aiogram.fsm.state import State, StatesGroup
from aiogram.fsm.storage.memory import MemoryStorage
from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup
from fastapi import FastAPI, Request
from openai import AsyncOpenAI

from config import get_settings
from database import (
    add_message_log,
    add_reviewed_patient,
    create_session,
    delete_reviewed_phone,
    get_active_session_by_phone,
    get_all_reviewed_phones,
    get_session_by_custom_id,
    get_session_by_id,
    get_session_by_lead_id,
    get_session_messages,
    has_patient_already_reviewed,
    init_db,
    normalize_phone,
    update_session,
)
from umnico_client import UmnicoClient, UmnicoError, extract_lead_id

settings = get_settings()
router = Router()
app = FastAPI(title="CityClinic Feedback Bot + Umnico Webhook")
telegram_bot: Bot | None = None


class AdminStates(StatesGroup):
    waiting_patient_data = State()


def get_umnico_client() -> UmnicoClient:
    if not settings.umnico_api_token:
        raise RuntimeError("Не задан UMNICO_API_TOKEN в Railway Variables")
    return UmnicoClient(settings.umnico_api_token, settings.umnico_api_base_url)


def parse_patient_data(text: str) -> tuple[str, str] | None:
    parts = [part.strip() for part in text.split(",", maxsplit=1)]
    if len(parts) != 2:
        return None
    patient_name, phone = parts
    if not patient_name or not phone:
        return None
    return patient_name, phone


def admin_help_text() -> str:
    return (
        "Команды администратора:\n\n"
        "/new — начать отправку пациенту в WhatsApp\n"
        "/id — показать ID текущего Telegram-чата\n"
        "/reviewed — показать номера, которые уже оставляли отзыв\n"
        "/delete_reviewed +79991234567 — удалить номер из базы оставивших отзыв\n"
        "/umnico_integrations — показать интеграции Umnico и их saId\n"
        "/setup_umnico_webhook — создать webhook в Umnico\n"
        "/umnico_webhooks — показать webhooks Umnico"
    )


def yandex_instruction(review_text: str) -> str:
    return (
        "Спасибо! Ниже текст Вашего отзыва.\n\n"
        f"{review_text}\n\n"
        "Нажмите и удерживайте текст отзыва выше, затем выберите «Скопировать».\n"
        "После этого откройте Яндекс.Карты по ссылке ниже.\n\n"
        "Пожалуйста, выберите 5 ⭐⭐⭐⭐⭐,\n"
        "в поле «опишите минусы и плюсы» нажмите «вставить».\n"
        "Текст Вашего отзыва будет вставлен.\n"
        "Опубликуйте отзыв 😊\n\n"
        f"{settings.yandex_maps_review_url}"
    )


def first_whatsapp_message(patient_name: str) -> str:
    return (
        f"{patient_name}, спасибо Вам, что посетили {settings.clinic_name}!\n"
        "Мы надеемся, что Вы остались довольны нашей работой.\n"
        "Оцените, пожалуйста, Ваш визит от 1 до 5.\n\n"
        "Ответьте одной цифрой: 1, 2, 3, 4 или 5."
    )


def positive_review_request() -> str:
    return (
        "Оставьте, пожалуйста, отзыв о нашей работе.\n"
        "Напишите Ваш отзыв следующим сообщением или отправьте голосовое сообщение."
    )


def negative_feedback_request() -> str:
    return (
        "Напишите, пожалуйста, подробнее о Ваших впечатлениях и замечаниях.\n"
        "Можно написать текстом или отправить голосовое сообщение."
    )


def normalize_rating(text: str) -> int | None:
    cleaned = (text or "").strip()
    if cleaned in {"1", "2", "3", "4", "5"}:
        return int(cleaned)
    return None


async def send_to_admin_group(bot: Bot, session_id: int, title: str) -> None:
    session = await get_session_by_id(session_id)
    messages = await get_session_messages(session_id)
    if session:
        patient_block = (
            f"<b>{html.escape(title)}</b>\n\n"
            f"<b>Канал:</b> WhatsApp через Umnico\n"
            f"<b>Пациент:</b> {html.escape(session['patient_name'])}\n"
            f"<b>Телефон:</b> {html.escape(session['phone'])}\n"
            f"<b>Оценка:</b> {session.get('rating') or 'не указана'}\n"
            f"<b>Umnico leadId:</b> {session.get('umnico_lead_id') or 'не указан'}\n\n"
        )
    else:
        patient_block = f"<b>{html.escape(title)}</b>\n\n"
    log_lines = []
    for item in messages:
        sender = "Бот" if item["sender"] == "bot" else "Пациент"
        log_lines.append(f"<b>{sender}:</b> {html.escape(item['text'])}")
    text = patient_block + "<b>Переписка:</b>\n" + "\n\n".join(log_lines)
    for i in range(0, len(text), 3900):
        await bot.send_message(settings.admin_group_id, text[i:i + 3900])


async def umnico_send_to_patient(session: dict, text: str) -> Any:
    client = get_umnico_client()
    lead_id = session.get("umnico_lead_id")
    custom_id = f"cityclinic-{session['id']}-{uuid4().hex[:8]}"

    if lead_id and settings.umnico_source_id and settings.umnico_user_id:
        return await client.send_lead_text(
            lead_id=int(lead_id),
            text=text,
            source_id=settings.umnico_source_id,
            user_id=settings.umnico_user_id,
            custom_id=custom_id,
        )

    if not settings.umnico_sa_id:
        raise RuntimeError("Не задан UMNICO_SA_ID. Его можно посмотреть командой /umnico_integrations")

    return await client.write_first_text(
        destination=normalize_phone(session["phone"]),
        text=text,
        sa_id=settings.umnico_sa_id,
        custom_id=custom_id,
    )


async def transcribe_telegram_voice(bot: Bot, voice: types.Voice) -> str:
    if not settings.openai_api_key:
        raise RuntimeError("OPENAI_API_KEY не задан. Распознавание голоса отключено.")
    client = AsyncOpenAI(api_key=settings.openai_api_key)
    file = await bot.get_file(voice.file_id)
    tmp_dir = Path("tmp_voice")
    tmp_dir.mkdir(exist_ok=True)
    ogg_path = tmp_dir / f"{voice.file_unique_id}.ogg"
    await bot.download_file(file.file_path, destination=ogg_path)
    with ogg_path.open("rb") as audio_file:
        result = await client.audio.transcriptions.create(model="whisper-1", file=audio_file, language="ru")
    try:
        os.remove(ogg_path)
    except OSError:
        pass
    return result.text.strip()


async def transcribe_audio_url(url: str) -> str:
    if not settings.openai_api_key:
        raise RuntimeError("OPENAI_API_KEY не задан. Распознавание голоса отключено.")
    tmp_dir = Path("tmp_voice")
    tmp_dir.mkdir(exist_ok=True)
    file_path = tmp_dir / f"umnico_{uuid4().hex}.ogg"
    async with httpx.AsyncClient(timeout=60) as client:
        response = await client.get(url)
        response.raise_for_status()
        file_path.write_bytes(response.content)
    openai_client = AsyncOpenAI(api_key=settings.openai_api_key)
    with file_path.open("rb") as audio_file:
        result = await openai_client.audio.transcriptions.create(model="whisper-1", file=audio_file, language="ru")
    try:
        os.remove(file_path)
    except OSError:
        pass
    return result.text.strip()


async def finish_positive_review(session: dict, review_text: str) -> None:
    session_id = session["id"]
    review_text = review_text.strip()
    await update_session(session_id, review_text=review_text, status="positive_finished")
    await add_message_log(session_id, "patient", review_text)
    await add_reviewed_patient(
        phone=session["phone"],
        patient_name=session["patient_name"],
        session_id=session_id,
        review_text=review_text,
        rating=session.get("rating"),
    )

    instruction = yandex_instruction(review_text)
    await add_message_log(session_id, "bot", instruction)
    await umnico_send_to_patient(session, instruction)

    final_text = "Спасибо большое за Ваш отзыв! Ждем Вас снова!"
    await add_message_log(session_id, "bot", final_text)
    await umnico_send_to_patient(session, final_text)

    if telegram_bot:
        await send_to_admin_group(telegram_bot, session_id, "Положительный отзыв пациента")


async def finish_negative_feedback(session: dict, feedback_text: str) -> None:
    session_id = session["id"]
    feedback_text = feedback_text.strip()
    await update_session(session_id, review_text=feedback_text, status="negative_finished")
    await add_message_log(session_id, "patient", feedback_text)
    await add_reviewed_patient(
        phone=session["phone"],
        patient_name=session["patient_name"],
        session_id=session_id,
        review_text=feedback_text,
        rating=session.get("rating"),
    )

    final_text = (
        "Спасибо большое за Ваш отзыв! Нам очень важно Ваше мнение, "
        "мы будем стараться сделать нашу работу лучше."
    )
    await add_message_log(session_id, "bot", final_text)
    await umnico_send_to_patient(session, final_text)

    if telegram_bot:
        await send_to_admin_group(telegram_bot, session_id, "Отзыв пациента с оценкой ниже 5")


async def process_whatsapp_message(session: dict, text: str) -> None:
    status = session.get("status")
    session_id = session["id"]

    if status == "waiting_rating":
        rating = normalize_rating(text)
        if rating is None:
            bot_text = "Пожалуйста, ответьте одной цифрой от 1 до 5."
            await add_message_log(session_id, "patient", text)
            await add_message_log(session_id, "bot", bot_text)
            await umnico_send_to_patient(session, bot_text)
            return

        await update_session(session_id, rating=rating)
        await add_message_log(session_id, "patient", f"Оценка: {rating}")
        if rating == 5:
            bot_text = positive_review_request()
            await update_session(session_id, status="waiting_positive_review")
        else:
            bot_text = negative_feedback_request()
            await update_session(session_id, status="waiting_negative_feedback")
        await add_message_log(session_id, "bot", bot_text)
        fresh_session = await get_session_by_id(session_id) or session
        await umnico_send_to_patient(fresh_session, bot_text)
        return

    if status == "waiting_positive_review":
        await finish_positive_review(session, text)
        return

    if status == "waiting_negative_feedback":
        await finish_negative_feedback(session, text)
        return

    # Если сессия уже завершена, просто вежливо отвечаем и не запускаем сценарий заново.
    bot_text = "Спасибо! Ваш отзыв уже получен."
    await add_message_log(session_id, "patient", text)
    await add_message_log(session_id, "bot", bot_text)
    await umnico_send_to_patient(session, bot_text)


def extract_incoming_text(payload: dict) -> str | None:
    message = payload.get("message") or {}
    data = message.get("data") or {}
    text = data.get("text") or message.get("text") or payload.get("text")
    if isinstance(text, str) and text.strip():
        return text.strip()
    return None


def extract_audio_url(payload: dict) -> str | None:
    message = payload.get("message") or {}
    data = message.get("data") or {}
    attachments = data.get("attachments") or message.get("attachments") or []
    if isinstance(attachments, dict):
        attachments = [attachments]
    for item in attachments:
        if not isinstance(item, dict):
            continue
        item_type = str(item.get("type") or "").lower()
        url = item.get("url") or item.get("src")
        if url and item_type in {"audio", "voice", "ptt", "doc"}:
            return str(url)
    return None


def extract_phone_from_payload(payload: dict) -> str | None:
    for key in ("phone", "chatId", "destination"):
        value = payload.get(key)
        if isinstance(value, str) and normalize_phone(value):
            return value
    customer = payload.get("customer") or {}
    if isinstance(customer, dict):
        value = customer.get("phone") or customer.get("login")
        if isinstance(value, str) and normalize_phone(value):
            return value
    lead = payload.get("lead") or {}
    if isinstance(lead, dict):
        customer = lead.get("customer") or {}
        if isinstance(customer, dict):
            value = customer.get("phone") or customer.get("login")
            if isinstance(value, str) and normalize_phone(value):
                return value
    return None


@app.get("/health")
async def health() -> dict:
    return {"ok": True, "service": "cityclinic-feedback-umnico"}


@app.post("/umnico/webhook")
async def umnico_webhook(request: Request) -> dict:
    payload = await request.json()
    event_type = payload.get("type")
    lead_id = payload.get("leadId")

    # Umnico может прислать исходящее сообщение с customId — сохраняем leadId, если он появился только сейчас.
    if event_type == "message.outgoing":
        custom_id = None
        message = payload.get("message") or {}
        if isinstance(message, dict):
            custom_id = message.get("customId") or message.get("custom_id")
        custom_id = custom_id or payload.get("customId")
        if custom_id and lead_id:
            session = await get_session_by_custom_id(str(custom_id))
            if session and not session.get("umnico_lead_id"):
                await update_session(session["id"], umnico_lead_id=int(lead_id))
        return {"ok": True}

    if event_type != "message.incoming":
        return {"ok": True, "ignored": event_type}

    session = None
    if lead_id:
        session = await get_session_by_lead_id(int(lead_id))

    if not session:
        phone = extract_phone_from_payload(payload)
        if phone:
            session = await get_active_session_by_phone(phone)
            if session and lead_id and not session.get("umnico_lead_id"):
                await update_session(session["id"], umnico_lead_id=int(lead_id))
                session = await get_session_by_id(session["id"])

    if not session:
        print(f"Webhook ignored: no active session for payload={payload}")
        return {"ok": True, "ignored": "no_active_session"}

    text = extract_incoming_text(payload)
    if not text:
        audio_url = extract_audio_url(payload)
        if audio_url:
            try:
                text = await transcribe_audio_url(audio_url)
            except Exception as exc:
                print(f"Umnico voice transcription error: {exc}")
                await umnico_send_to_patient(session, "Не получилось распознать голосовое сообщение. Пожалуйста, отправьте текстом.")
                return {"ok": True, "error": "voice_transcription_failed"}

    if not text:
        await umnico_send_to_patient(session, "Пожалуйста, отправьте текстовое или голосовое сообщение.")
        return {"ok": True, "ignored": "no_text"}

    await process_whatsapp_message(session, text)
    return {"ok": True}


@router.message(CommandStart())
async def start(message: types.Message, state: FSMContext) -> None:
    await state.clear()
    await message.answer(admin_help_text())


@router.message(Command("new"))
async def new_feedback(message: types.Message, state: FSMContext) -> None:
    await state.set_state(AdminStates.waiting_patient_data)
    await message.answer(
        "Отправьте данные пациента в формате:\n\n"
        "<code>Иван Иванович, +79991234567</code>\n\n"
        "После этого сообщение пациенту уйдет в WhatsApp через Umnico."
    )


@router.message(Command("id"))
async def get_chat_id(message: types.Message) -> None:
    await message.answer(f"Chat ID: <code>{message.chat.id}</code>")


@router.message(Command("reviewed"))
async def show_reviewed(message: types.Message) -> None:
    rows = await get_all_reviewed_phones()
    if not rows:
        await message.answer("В базе пока нет номеров пациентов, уже оставивших отзыв.")
        return
    lines = ["Номера пациентов, уже оставивших отзыв:"]
    for row in rows[:100]:
        lines.append(
            f"• {html.escape(row['original_phone'])} — {html.escape(row.get('patient_name') or 'без имени')} "
            f"— оценка: {row.get('rating') or '—'}"
        )
    if len(rows) > 100:
        lines.append(f"\nПоказаны первые 100 из {len(rows)} записей.")
    await message.answer("\n".join(lines))


@router.message(Command("delete_reviewed"))
async def delete_reviewed(message: types.Message) -> None:
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) < 2:
        await message.answer("Укажите номер после команды, например:\n<code>/delete_reviewed +79991234567</code>")
        return
    deleted = await delete_reviewed_phone(parts[1])
    if deleted:
        await message.answer(f"Номер удален из базы: <code>{html.escape(parts[1])}</code>")
    else:
        await message.answer("Такого номера в базе не найдено.")


@router.message(Command("umnico_integrations"))
async def umnico_integrations(message: types.Message) -> None:
    try:
        data = await get_umnico_client().list_integrations()
    except Exception as exc:
        await message.answer(f"Ошибка Umnico API: <code>{html.escape(str(exc))}</code>")
        return
    if not data:
        await message.answer("Интеграции Umnico не найдены.")
        return
    lines = ["Интеграции Umnico:"]
    for item in data:
        if not isinstance(item, dict):
            continue
        lines.append(
            f"• id/saId: <code>{item.get('id')}</code>; "
            f"type: <code>{html.escape(str(item.get('type')))}</code>; "
            f"login: {html.escape(str(item.get('login') or ''))}; "
            f"status: {html.escape(str(item.get('status') or ''))}"
        )
    await message.answer("\n".join(lines[:50]))


@router.message(Command("setup_umnico_webhook"))
async def setup_umnico_webhook(message: types.Message) -> None:
    if not settings.public_base_url:
        await message.answer("Сначала задайте PUBLIC_BASE_URL в Railway, например https://your-app.up.railway.app")
        return
    url = f"{settings.public_base_url}/umnico/webhook"
    try:
        result = await get_umnico_client().create_webhook(url)
    except Exception as exc:
        await message.answer(f"Ошибка создания webhook: <code>{html.escape(str(exc))}</code>")
        return
    await message.answer(f"Webhook создан:\n<code>{html.escape(str(result))}</code>")


@router.message(Command("umnico_webhooks"))
async def umnico_webhooks(message: types.Message) -> None:
    try:
        result = await get_umnico_client().list_webhooks()
    except Exception as exc:
        await message.answer(f"Ошибка получения webhooks: <code>{html.escape(str(exc))}</code>")
        return
    await message.answer(f"Webhooks Umnico:\n<code>{html.escape(str(result))}</code>")


@router.message(AdminStates.waiting_patient_data, F.text)
async def receive_patient_data(message: types.Message, state: FSMContext) -> None:
    parsed = parse_patient_data(message.text)
    if not parsed:
        await message.answer(
            "Не получилось разобрать данные. Отправьте, пожалуйста, так:\n\n"
            "<code>Иван Иванович, +79991234567</code>"
        )
        return

    patient_name, phone = parsed
    phone_normalized = normalize_phone(phone)

    if await has_patient_already_reviewed(phone):
        await message.answer("Пациент с таким телефоном уже оставлял отзыв. Сообщение в WhatsApp не отправлено.")
        await state.clear()
        return

    session_id = await create_session(message.from_user.id, patient_name, phone, channel="whatsapp")
    session = await get_session_by_id(session_id)
    custom_id = f"cityclinic-start-{session_id}-{uuid4().hex[:8]}"
    await update_session(session_id, umnico_custom_id=custom_id)

    bot_text = first_whatsapp_message(patient_name)
    await add_message_log(session_id, "bot", bot_text)

    try:
        if not settings.umnico_sa_id:
            raise RuntimeError("Не задан UMNICO_SA_ID. Выполните /umnico_integrations и добавьте нужный id в Railway Variables.")
        response = await get_umnico_client().write_first_text(
            destination=phone_normalized,
            text=bot_text,
            sa_id=settings.umnico_sa_id,
            custom_id=custom_id,
        )
        lead_id = extract_lead_id(response)
        if lead_id:
            await update_session(session_id, umnico_lead_id=lead_id)
    except Exception as exc:
        await message.answer(f"Не удалось отправить сообщение в WhatsApp через Umnico:\n<code>{html.escape(str(exc))}</code>")
        return

    await state.clear()
    await message.answer(
        "Сообщение пациенту отправлено в WhatsApp через Umnico.\n"
        f"Пациент: <b>{html.escape(patient_name)}</b>\n"
        f"Телефон: <code>{html.escape(phone)}</code>"
    )


@router.message()
async def fallback(message: types.Message, state: FSMContext) -> None:
    current_state = await state.get_state()
    if current_state is None:
        await message.answer("Нажмите /new, чтобы начать сбор обратной связи через WhatsApp.")
    else:
        await message.answer("Пожалуйста, следуйте текущему шагу сценария.")


async def run_fastapi() -> None:
    port = int(os.getenv("PORT", "8000"))
    config = uvicorn.Config(app, host="0.0.0.0", port=port, log_level="info")
    server = uvicorn.Server(config)
    await server.serve()


async def run_telegram() -> None:
    global telegram_bot
    telegram_bot = Bot(
        token=settings.bot_token,
        default=DefaultBotProperties(parse_mode=ParseMode.HTML),
    )
    dp = Dispatcher(storage=MemoryStorage())
    dp.include_router(router)
    print("CityClinic Telegram admin bot started")
    await dp.start_polling(telegram_bot)


async def main() -> None:
    await init_db()
    print("CityClinic feedback service started")
    await asyncio.gather(run_fastapi(), run_telegram())


if __name__ == "__main__":
    asyncio.run(main())
