...
async def get_all_reviewed_patients() -> list[dict]:
    """
    Возвращает список пациентов, которые уже оставляли отзыв.
    Используется админ-командой /reviewed.
    """
    async with aiosqlite.connect(DB_PATH) as db:
...
async def delete_reviewed_patient(phone: str) -> bool:
    """
    Удаляет телефон из базы уже оставивших отзыв.
    Возвращает True, если номер был найден и удален.
    """
    phone_normalized = normalize_phone(phone)
    if not phone_normalized:
        return False
