...
async def create_session(user_id: int, patient_name: str, phone: str, channel: str = "whatsapp", wazzup_channel_id: str | None = None) -> int:
    now = datetime.utcnow().isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("""
            INSERT INTO feedback_sessions
            (user_id, patient_name, phone, phone_normalized, channel, wazzup_channel_id, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_id, patient_name, phone, normalize_phone(phone), channel, wazzup_channel_id, "waiting_rating", now, now))
        await db.commit()
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cursor = await db.execute(f"""
            ...
            WHERE phone_normalized = ?
              AND status IN ('waiting_rating', 'waiting_positive_review', 'waiting_negative_feedback')
              {where_channel}
